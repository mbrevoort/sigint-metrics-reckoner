...
Graphite.prototype._handleMessage = function _handleMessage (channel, message) {
    try {
      ...
      this._recurseValue(parsed.name, parsed.value, parsed.date);
    }
    catch(err) {
      util.log('Graphite._handleMessage ' + util.inspect(err));
    }
};

Graphite.prototype._recurseValue = function _recurseValue(name, value, date) {
  var self = this;
  
  if(typeof value === 'number') {
    this.carbonClient.write(name, value, date);
  } else if(typeof value === 'object' && value != null) {
    Object.keys(value).forEach(function(key) {
      var newName = name + '.' + key;
      var newValue = value[key];
      self._recurseValue(newName, newValue, date);
    });
  } else {
    util.log('Graphite._recurseValue skipped metric value: ', util.inspect(value, false, 10)); 
  }
}
