...
AMQPCollector.prototype._onQueue = function _onQueue(queue) {
  ...
  this._queue.subscribeRaw(options, function(m) {
        ...
        m.addListener('end', function() {
          that._onMessage(b, m.contentType);
          if(doAck) {
            m.acknowledge();
          }
        });
...
AMQPCollector.prototype._onMessage = function _onMessage(data, contentType) {

  var parsed = (contentType == 'application/json') ? JSON.parse(data) : BSON.parse(data),
      node = parsed.s.n,
      app = parsed.s.a,
      type = parsed.t,
      op = parsed.o,
