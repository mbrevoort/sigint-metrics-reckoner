...
Harvester.prototype._publish = function _publish(name, value, metric) {
  ...
  var type = self._inferType(value);
  var payload = {
    type:     type,
    name:     name,
    value:    value,
...
Harvester.prototype._inferType = function _inferType(metricJSON) {
  // TODO, just parse the type out of the name or something else
  if(!metricJSON) return null;
  else if(typeof metricJSON === 'number') return 'counter';
  else if(metricJSON.hasOwnProperty('currentRate')) return 'meter';
  else if(metricJSON.hasOwnProperty('stddev')) return 'histogram';
  else if(metricJSON.hasOwnProperty('meter') 
      && metricJSON.hasOwnProperty('histogram')) return 'timer';
  else return 'unknown';
};
